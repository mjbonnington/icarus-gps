...
def get_pools():
	...
	"""
	try:
		pools = os_wrapper.execute([os.environ['IC_DEADLINE_CMD_EXECUTABLE'], '-pools'])[1]
...
def get_groups():
	...
	"""
	try:
		groups = os_wrapper.execute([os.environ['IC_DEADLINE_CMD_EXECUTABLE'], '-groups'])[1]
...
def submit_job(**kwargs):
	...
	try:
		if kwargs['renderLayers']:  # Batch submission -----------------------
			...
			cmd_result, cmd_output = os_wrapper.execute([os.environ['IC_DEADLINE_CMD_EXECUTABLE'], batchSubmissionFile])
			if cmd_result:
			...
			# Execute deadlinecommand
			cmd_result, cmd_output = os_wrapper.execute([os.environ['IC_DEADLINE_CMD_EXECUTABLE'], jobInfoFile, pluginInfoFile])
			if cmd_result:
