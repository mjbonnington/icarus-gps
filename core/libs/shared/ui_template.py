...
#class TemplateUI(Sentinel):
class TemplateUI(object):
	""" Template UI class.
		Subclasses derived from this class need to also inherit QMainWindow or
		QDialog. This class has no __init__ method as a fudge to get around
		the idiosyncracies of multiple inheritance in Python 2 and 3.
	"""
	# def __init__(self, parent=None):
	...
	def setupUI(self, 
		...
		if ui_file:
			...
			####self.ui = QtCompat.load_ui(fname=uifile)
			self.ui = QtCompat.loadUi(uifile, self)
			##self.ui = setup_ui(uifile, self)
			#self.ui = baseinstance2.load_ui_wrapper(uifile, self)
