...
class AppLauncher(QtWidgets.QDialog):
	...
	def createIcon(self, app, iconSize, layout, column):
		...
		if submenus:
			...
			for entry in submenus:
				...
				# exec_str = []
				# exec_str.append("self.%s = QtWidgets.QAction(menuName, None)" %actionName)
				# exec_str.append("self.%s.setObjectName('%s')" %(actionName, actionName))
				# exec_str.append("self.%s.triggered.connect(lambda: self.launchApp(displayName, flags))" %actionName)
				# exec_str.append("toolButton.addAction(self.%s)" %actionName)
				# # print(exec_str)
				# for line in exec_str:
				# 	print(line)
				# 	exec(line) # execute lines individually?
	...
	def getRows(self, num_items):
		...
		if num_items <= 4:
			...
			num_rows = math.ceil(num_items/(5*1.0))

		max_row_width = math.ceil(num_items/(num_rows*1.0))

		rows = []
		for i in range(num_rows):
			rows.append(int(max_row_width))
	...
	# def launchApp(self, app, flags=None):
	# 	""" Launches an application.
	# 	"""
	# 	launchApps.launch(app, flags)
	# 	if self.parent.boolMinimiseOnAppLaunch:
