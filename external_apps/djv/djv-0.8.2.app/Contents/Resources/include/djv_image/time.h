...
struct _DJV_IMAGE Time : public file_seq::Seq
{
  ...
  Time();
  Time(const List<int> &, const Speed & = Speed());
  Time(int start, int end, const Speed & = Speed());
...
_DJV_IMAGE int timecode_to_frame(
  uint32_t timecode,
  const time::Speed &
);
...
_DJV_IMAGE String units_to_string(int frame, const Speed &);

_DJV_IMAGE int string_to_units(const String &, const Speed &);
